export default {
  common: {
    ...
    reset_when_wilddog_too_many_connections:
      "当前数据库超载，正在切换至备用数据库。注意：主数据库和备用数据库间数据不互通。",
    unknown_ip: "未知 IP 地址",
    unknown_user: "未知用户",
  ...
  AdminHelpers: {
    ...
    text: {
      desc_resetting_discussion_count_for_all_pages:
        "如果页面的评论数量出现错误的话，你可以通过这个辅助功能重置所有页面的评论数量。",
  ...
  PersonalCenter: {
    ...
    text: {
      ...
      new_comment_on_page:
        '有人在 <a href="{{pageURL}}" target="blank">{{pageTitle}}</a> 发布了新评论。',
      "new_comment_on_page+":
        '<a title="{{email}}">{{displayName}}</a> 在 <a href="{{pageURL}}" title="点击访问 {{pageTitle}}" target="blank">{{pageTitle}}</a> 发布了 <a title="{{content}}">新评论</a>。',
      new_reply_to_comment: "有人回复了你的评论。 ",
      "new_reply_to_comment+":
        '<a title="{{email}}">{{displayName}}</a> 对你的评论进行了<a title="{{content}}">回复</a>。',
      new_disc_in_comment: "有人在你的评论的讨论中进行了回复。",
      "new_disc_in_comment+":
        '<a title="{{email}}">{{displayName}}</a> 在你的评论的讨论中进行了 <a title="{{content}}">回复</a> 。',
      new_mention: "有人在评论中 @ 了你。",
      "new_mention+":
        '<a title="{{email}}">{{displayName}}</a> 在 <a title="{{content}}">评论</a> 中 @ 了你。',
